...
| 1.2a | Full-bleed header image | `s1-charismatic-leadership.jpg` | Alt: "Crowd raising hands toward a charismatic leader on a podium." Sits between the section title and Part I as a visual opener. |
...
| 1.5 | Weber portrait | Image | `s1-weber-portrait-1918.jpg` — small grayscale portrait, similar treatment to Herbert portrait in Hero. Alt: "Max Weber, 1918." |
| 1.6 | **Weber three-authority animated bars** | Interactive visualization | Three equal-height vertical bars labeled **Tradition**, **Law**, **Charisma**. On load, all three are dark gray except Charisma, which is red and pulses with a glowing halo. When the bars scroll into view: Charisma expands to ~2× height and Tradition + Law collapse to stubs with a diagonal striped "broken" texture overlaid — visually Charisma crushes the other two. |
...
| 1.10 | Routinization paragraph | Prose | `But Weber saw further than the moment of seizure. He argued that charismatic authority is inherently unstable — it cannot sustain itself beyond the leader's lifetime. It must undergo what he called routinization: either it hardens into bureaucratic rule, fractures into competing traditions, or the institutions hold and charisma simply fades. What follows in the next sections are three variations of that process — and the question of whether there is a fourth.` |
| 1.11 | **Routinization pathway diagram** | Animated SVG | A single node labeled **Charisma** at top, with three branching paths downward. Path A: `Routinized into Bureaucracy` (labeled `Lenin → Stalin`). Path B: `Fractured into Competing Traditions` (labeled `Muhammad → The Schism`). Path C: `Constrained by Institutions` (labeled `?`, dimmed, with a subtle lock icon). On scroll, paths A and B illuminate sequentially; path C stays dim with a pulsing `?` — it is answered in the Takeaway. |
...
| Step 2 | `Red Guard factions turn on each other. Provincial governments collapse. Liu Shaoqi — the second most powerful man in China — is dragged from his home and left to die in detention. The revolution is eating its own.` Image pair: `s1-anti-liu-shaoqi-poster.jpg` (alt: "Anti-Liu Shaoqi propaganda poster during the Cultural Revolution"; caption: "Liu Shaoqi, denounced as 'China's Khrushchev.' Died in detention, 1969.") and `s1-peng-dehuai-military-portrait.jpg` (alt: "Marshal Peng Dehuai in military uniform before persecution"; caption: "Peng Dehuai — defense minister, war hero — before he was purged."). Followed by a single full-width image `s1-peng-dehuai-struggle-session.jpg` (alt: "Marshal Peng Dehuai subjected to a struggle session by Red Guards"; caption: "The same man, after. Beaten until his back was broken for telling Mao the Great Leap was failing. Died in prison, 1974.") — forming a before/after pair with the military portrait. | Year `1967`, event `Factions turn on each other. Governments collapse.`, intensity 55%. |
...
| 2A.14 | **Purge statistics counter** | Two large red numbers animating from 0 on entry: `Executed` (final value from JS: purgeExec target), `Sent to Gulag` (final value from JS: purgeGulag target). |
...
| 2A.18 | Lenin's Testament blockquote | `"Stalin is too rude... I propose to find a way to remove Stalin from that position and appoint to it another man who is more patient, more loyal, more polite."` — cite: `— Lenin's Testament, January 4, 1923` |
...
| 2B.4 | Image | `s2-muhammud.jpg` — alt: "The early Islamic world" — caption: "The early Islamic world". |
...
| 2B.6 | Full-bleed image | `s2-karbala.webp` — alt: "The Battle of Karbala, 680 CE". Stretches edge-to-edge. |
...
| 2B.9 | Image | `s2-abura.jpeg` — alt: "Ashura commemoration" — caption: "Ashura commemoration — mourning Husayn, 1400 years later". |
| 2B.10 | **Islamic schism branch tree (interactive SVG)** | A top-down family tree showing the fracture of Islam into branches. Hovering any node surfaces a tooltip with a title and explanation. Full node list below. |
...
| # | Element | Content |
|---|---------|---------|
| 3.8 | Bridge paragraph 1 | `Festinger described a defense mechanism inside one person's head. But people do not believe alone. Cass Sunstein studied what happens when people who share a belief are placed together: the group does not settle at the average of its members' views. It moves toward the extreme. He called this group polarization — and he demonstrated it in jury deliberations, political caucuses, and online forums. The mechanism is simple: in a room where everyone agrees, the most forceful voice sets the new anchor. Moderate positions feel like betrayal.` |
| 3.9 | **Polarization drift simulation** | Interactive scroll-driven visualization. A field of ~60 small dots, initially scattered randomly across a horizontal spectrum (left to right = opinion range). As the user scrolls: **Phase 1** (Festinger — individual): dots are randomly placed, each a slightly different shade. Label: `Individual beliefs — scattered`. **Phase 2** (Sunstein — group): dots drift toward 3–4 clusters, leaving empty space in the center. Clusters tighten. Label: `Group polarization — like attracts like, moderates vanish`. **Phase 3** (Pariser — algorithm): each cluster drifts further toward the edges. A semi-transparent bubble border appears around each cluster. Label: `Filter bubbles — the algorithm locks the door`. Final state: two dense clusters at far left and far right, wide empty center, each enclosed in a bubble. |
| 3.10 | Bridge paragraph 2 | `Eli Pariser identified the infrastructure that automates Sunstein's process. Social media algorithms optimize for engagement. Devotion engages. Doubt does not. The filter bubble is an environment where the information you see is pre-selected to confirm what you already believe. The technology is new. The individual psychology is Festinger's. The group dynamics are Sunstein's. The scale is what changed.` |
...
## SECTION 5 — Takeaway: When the Cages Held
...
| 5.8 | "Boring is why it works" paragraphs | `Nobody chants for judicial independence. There are no rallies for procedural checks. You cannot build a mass movement around the separation of powers. These things are boring. That is why they work.` / `But do they? Everything you have read so far is a story of cages that failed. Institutions that yielded, followers who sealed themselves, movements that outlived their founders and became something worse. It would be easy to conclude that the cages never hold.` / `They have held. Three times in the twentieth century, charismatic leaders met institutional resistance — and the institutions won.` |
...
| 5.10 | De Gaulle paragraphs | `Charles de Gaulle was the most powerful democratic leader in postwar Europe. He had liberated France, founded the Fifth Republic, and written a constitution that concentrated executive power in the presidency — around himself. He was, by any definition, a charismatic leader who had reshaped his country's institutions in his own image.` / `In April 1969, de Gaulle put a package of constitutional reforms to a national referendum. He announced in advance that if the vote failed, he would resign. It was a gamble — the same kind of gamble Hitler had made in 1933, staking everything on a single vote. The difference was the outcome.` Between the two paragraphs, image `takeaway-degaulle-speech.jpg` (alt: "Charles de Gaulle addressing journalists at a press conference"; caption: "De Gaulle in office — the constitution he wrote put him at the center of the Fifth Republic."). After the second paragraph, image `takeaway-degaulle-voting.jpg` (alt: "Charles de Gaulle casting his vote in the 1969 referendum"; caption: "De Gaulle casts his own vote, April 27, 1969."). |
...
| 5.11a | Image | `takeaway-degaulle-referendum.jpeg` — alt: "Newspaper headlines reporting the results of the 1969 French referendum" — caption: `The morning after: a 'Non' the founder of the Republic chose to honor.` Sits between the referendum bar and the resolution paragraph. |
| 5.12 | De Gaulle resolution | `The French voted Non. De Gaulle resigned the next day. No tanks, no emergency powers, no attempt to circumvent the result. The man who had built the cage walked into it and closed the door behind him.` / `The Fifth Republic continued without him. The institutions he had designed survived the departure of the personality they had been designed around. Weber would have recognized the pattern: charismatic authority, successfully routinized into law.` |
...
| 5.15 | FDR paragraphs | `Franklin Roosevelt won the 1936 presidential election with 61% of the vote — the largest popular mandate since James Monroe ran unopposed. He carried 46 of 48 states. His New Deal had reshaped the American economy. His charisma was immense, his popularity unrivaled.` / `And the Supreme Court kept striking down his laws. So in February 1937, Roosevelt proposed the Judicial Procedures Reform Bill — a plan to add up to six new justices to the Court, one for every sitting justice over the age of seventy. The transparent goal: pack the bench with allies who would stop blocking the New Deal.` |
| 5.16 | **Supreme Court bench diagram** | Interactive scroll-triggered visualization. Nine seat icons in a row (the existing Court), each labeled with a justice's name. As user scrolls: six additional seats appear to the right in dashed outlines (FDR's proposed additions). Then — one by one — the dashed seats crack and disappear, accompanied by a counter: `Senate vote: 70 against, 22 in favor`. The nine original seats remain, unchanged. Visual message: the institution held its shape. |
| 5.17 | Image | `takeaway-fdr-supreme-court-1937.png` — alt: "The nine justices of the U.S. Supreme Court, 1937" — caption: "The Hughes Court, 1937. Nine seats. Roosevelt wanted fifteen. The Senate said no." |
| 5.18 | FDR resolution | `The Senate — controlled by Roosevelt's own party — killed the bill 70 to 22. Democrats voted against their own president, against the most popular leader in American history, because the institutional principle mattered more than the political alliance. Roosevelt remained president. He won two more elections. But the Court stayed at nine.` / `The cage held — not because Roosevelt lacked charisma, but because the people inside the institution chose the institution over the leader. That choice is never guaranteed. It has to be made, every time, by specific people in a specific moment.` |

### Counter-example 3 — Mandela, 1999

| # | Element | Content |
|---|---------|---------|
| 5.19 | Image | `takeaway-mandela-salute.jpg` — alt: "Nelson Mandela saluting a crowd" — caption: "Nelson Mandela, Galeshewe, South Africa." |
| 5.20 | Mandela paragraphs | `Nelson Mandela spent twenty-seven years in prison and emerged to lead the dismantling of apartheid. He won South Africa's first democratic election in 1994 with 62% of the vote. His moral authority was absolute. No one would have objected to a second term — or a third.` / `He served one term and stepped down. In a continent where leaders routinely clung to power for decades, Mandela walked away from the presidency at the height of his influence. He chose the institution over himself.` |
| 5.21 | Mandela reflection | `But this is the hardest case — and we should be honest about why. De Gaulle was constrained by a vote. Roosevelt was constrained by the Senate. Mandela was constrained by nothing except his own judgment. If a different person had stood where Mandela stood — someone with the same charisma but less restraint — would South Africa's young institutions have held?` / `The answer is not obvious. And that is the point. Institutions are necessary but not sufficient. They must be built, and then they must be chosen — again and again, by people who have the power to ignore them.` |
| 5.22 | Image | `takeaway-mandela-portrait.jpg` — alt: "Portrait of Nelson Mandela" — caption: "The institution did not constrain him. He constrained himself. That is the exception, not the rule." |

### Synthesis

| # | Element | Content |
|---|---------|---------|
| 5.23 | **Three-column comparison table** | Scroll-triggered fade-in. Three cards side by side, each with identical structure. Header row: `Leader` / `Pressure Moment` / `What Held` / `Outcome`. Card 1: `De Gaulle` / `1969 referendum` / `The vote` / `Resigned. Republic continued.` Card 2: `Roosevelt` / `Court-packing, 1937` / `The Senate (his own party)` / `Bill killed 70–22. Court stayed at nine.` Card 3: `Mandela` / `Presidency, 1999` / `His own restraint` / `Stepped down. But — would the next leader?` The third card's "What Held" cell has a subtle question-mark pulse, visually distinct from the first two. |
| 5.24 | Synthesis paragraphs | `The pattern is not "good leaders versus bad leaders." It is: are there structures that hold when the pressure comes? De Gaulle designed a constitution and then submitted to it. Roosevelt's own party chose the judiciary over their president. Mandela chose the institution over himself — but his case is the fragile one, because it depended on one man's character rather than one system's design.` / `Every catastrophe in the preceding sections — the Enabling Act, the Cultural Revolution, the Great Purge, the Great Famine — happened because at the critical moment, no institution was strong enough, or no person inside the institution was willing, to say no.` / `The cages do not hold by themselves. They hold because someone, at some point, decides they matter more than the leader standing in front of them.` |
...
> `The next charismatic leader will come. They always do. The cages have held before. Whether they hold again depends on whether anyone chooses to keep them closed.`
...
12. Cass Sunstein, *Going to Extremes: How Like Minds Unite and Divide* (2009)
13. Julian Jackson, *De Gaulle* (2018)
14. Jeff Shesol, *Supreme Power: Franklin Roosevelt vs. the Supreme Court* (2010)
15. Nelson Mandela, *Long Walk to Freedom* (1994)
...
| `s1-weber-portrait-1918.jpg` | S1 Theory | Max Weber, 1918. |
...
| `s1-peng-dehuai-struggle-session.jpg` | S1 Part III, step 2 | Marshal Peng Dehuai subjected to a struggle session by Red Guards. |
| `s1-peng-dehuai-military-portrait.jpg` | S1 Part III, step 2 (paired with struggle-session image as before/after) | Peng Dehuai in military uniform before persecution. |
| `s1-mao-tiananmen.jpg` | S1 Part III, step 0 (second image after Red Guards rally) | Mao reviews the Red Guards from Tiananmen Gate, August 1966. |
...
| `s1-charismatic-leadership.jpg` | S1 full-bleed header image (between section title and Part I) | Crowd raising hands toward a charismatic leader on a podium. |
...
| `takeaway-degaulle-speech.jpg` | Takeaway, counter-example 1 | De Gaulle at a press conference, between De Gaulle paragraphs. |
| `takeaway-degaulle-referendum.jpeg` | Takeaway, counter-example 1 (row 5.11a) | Newspaper headlines reporting the 1969 referendum result, between referendum bar and resolution paragraph. |
| `takeaway-degaulle-voting.jpg` | Takeaway, counter-example 1 | De Gaulle voting, placed before the referendum bar. |
...
3. **Routinization pathway diagram (S1 Theory)** — three branching paths from charisma: routinized into bureaucracy (Lenin→Stalin), fractured into competing traditions (Muhammad), or constrained by institutions (answered in Takeaway).
4. **Reichstag hemicycle (S1 Enabling)** — visually reconstructs the 441-to-94 vote: the chamber fills red as institutional resistance collapses.
5. **Cultural Revolution timeline (S1 Fire)** — year-by-year escalation from 1966 to 1976, with intensity building to 100%, a causal annotation explaining institutional collapse, and a final death-toll line.
6. **Testament manuscript interactive (S2 Case A)** — a Russian-language document image that reveals an English translation overlay on click, RE2-remake style. The "remove Stalin" line is highlighted red.
7. **Before/after photo sliders (S2 Case A)** — interactive proof that the Soviet state edited Trotsky and Yezhov out of existence in its own photographic record.
8. **Purge stat counters + victim grid (S2 Case A)** — quantifies the Great Purge in executed / gulag numbers and a dot grid.
9. **Islamic branch tree (S2 Case B)** — static SVG tree listing every major Sunni/Shia branch with hover tooltips; culminates in a death toll of 1,850,000+ across four modern sectarian wars.
10. **632 → 2026 sticky counter (S2 end)** — visually equates the passage of 1,394 years with a cumulative death toll climbing from 0 to 1,850,000. Sub-label now acknowledges each conflict had its own political triggers.
11. **Inflate scrolly (S3 Great Leap Forward)** — visually dramatizes the harvest number growing 100 → 200 → 350 → 600 → 1,000 tons as each layer of the bureaucracy lies upward, then collapsing into famine.
12. **Famine counter (S3)** — counts up to an estimate of the Great Famine death toll (1959–61), with causal annotation and famine photograph.
13. **Dissonance demo (S3 Festinger)** — a two-button choice that delivers the same answer regardless: "In 1959, every official chose 'Reject.'"
14. **Polarization drift simulation (S3 bridge)** — ~60 dots drift from random scatter → group clusters → extreme-edge bubbles, illustrating Festinger → Sunstein → Pariser causal chain.
15. **Trial list (S3 Confession)** — five-row table of Old Bolsheviks and their verdicts, alongside Bukharin's portrait.
16. **Transition zone** — pure visual scrub from dark mode to light mode.
17. **De Gaulle referendum bar (Takeaway)** — horizontal bar showing 52.41% Non / 47.59% Oui. Visual mirror of the hemicycle vote — but this time, the vote constrains the leader.
18. **FDR Supreme Court bench diagram (Takeaway)** — nine seats + six dashed proposed seats that crack and vanish as the Senate kills the bill 70–22.
19. **Three-column comparison table (Takeaway synthesis)** — De Gaulle / FDR / Mandela side by side: Leader, Pressure Moment, What Held, Outcome. Mandela's "What Held" column pulses with a question mark.
20. **Closing** — word-by-word reveal of a single line.
