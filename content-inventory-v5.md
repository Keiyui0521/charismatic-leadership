...
8. **Year counter redesigned as horizontal timeline** — The old vertical scroll-driven counter had two bugs: (a) numbers lost centering as digit count grew, (b) the section often scrolled away before reaching 2020. v5 replaces it with a **horizontal timeline** pinned to the viewport. A progress indicator moves left-to-right from 632 CE to 2024. At each major conflict, a timestamp marker appears with the event name and death toll. The cumulative number is displayed above the timeline and updates as the marker advances. The horizontal layout eliminates the centering bug, and the pinned sticky behavior eliminates the premature-scroll-away bug.
...
| B6a | Paul Atreides image — **MOVED BEFORE B6 (v5)** | `opening-paul-atreides.png` | Film still of Timothée Chalamet as Paul Atreides in Dune (2021). Alt: "Paul Atreides in Dune (2021)." Caption: `Paul Atreides — the hero of Frank Herbert's Dune.` **This image now appears BEFORE the Herbert paragraph, not after it.** The photo provides a visual anchor; the caption identifies the character for readers who haven't seen the film. Then the Herbert text below explains why it matters. |
...
### 2C — 632 → 2024 horizontal death-toll timeline — **REDESIGNED (v5)**

**v5 redesign rationale:** The old vertical scroll-driven counter had two bugs: (a) the large red number lost centering as digit count grew (layout shift), and (b) the section frequently scrolled away before reaching 2020 because the scroll zone was too short relative to the data range. v5 replaces the entire module with a **horizontal timeline**.

A full-viewport sticky scroll sequence (250vh tall) pinned while the user scrolls.

| # | Element | Behavior |
|---|---------|----------|
| 2C.1 | **Horizontal timeline axis** | A thin horizontal line spanning ~80% of viewport width. Left end labeled `632 CE`, right end labeled `2024`. The line is divided into timestamp markers at each major conflict year. The axis is pinned to the lower-center of the viewport. |
| 2C.2 | **Progress indicator** | A red dot or vertical red marker that moves from left to right along the timeline as the user scrolls. Its horizontal position maps linearly to the year range (632–2024). |
| 2C.3 | **Cumulative death toll (large red number)** | Displayed ABOVE the timeline, centered. This number updates as the progress indicator advances: roughly 10,000 by 680 (Karbala era), 80,000 by 750 (Umayyad civil wars), 120,000 by 1500, 150,000 by 1980, jumps sharply to ~1,150,000 after the Iran-Iraq War (1988), 1,400,000 after the Iraq civil war (2011), 1,700,000 after the Syrian war (2014), and finishes at **1,850,000+** at 2024. **Use `font-variant-numeric: tabular-nums` and a fixed container width to prevent layout shift as digit count grows.** |
| 2C.4 | **Timestamp event markers** | When the progress indicator reaches a major conflict year, a vertical marker rises from the timeline with the event name and individual death toll. Markers stay visible (fading to muted) as the indicator passes them, building a visual record of accumulation. Events: `680 — Karbala & early succession wars (~10,000)`, `750 — Umayyad civil wars (~70,000)`, `1988 — Iran-Iraq War (~1,000,000)`, `2011 — Iraq Civil War (~200,000)`, `2014 — Syrian Civil War (~500,000)`, `2024 — Yemen Civil War (~150,000+)`. |
| 2C.5 | Sub-label | Italic `killed in conflicts with sectarian dimensions — each triggered by its own political crisis, none reducible to religion alone`. Displayed below the timeline axis. |
