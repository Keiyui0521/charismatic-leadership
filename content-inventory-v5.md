# The Dark Side of Charismatic Leadership — Complete Content Inventory (v5)
...
Changes driven by 修改意见 ver2 review:

1. **Flash-card scroll architecture (G3 rewrite)** — The old G3 implementation (large `margin-bottom` between `.fade-in` elements) did not reliably prevent multiple elements from being visible at once. v5 replaces it entirely with a **GSAP ScrollTrigger pin-based flash-card system**: every prose block, image, and standalone element is individually pinned to the viewport for its scroll duration, then fades out as the next element fades in. The reader sees ONE element at a time — no exceptions. Previous and next content are invisible. Think of the page as a stack of flash cards. Scrolly modules (hemicycle, CR timeline, inflate number, polarization) retain their existing sticky-scroll behavior but must also respect the flash-card boundary: when a scrolly module ends, nothing from the next section should be visible until the user scrolls past the transition.
2. **Global font size increase** — `body font-size` increased from 18px → 20px. All relative sizes bumped proportionally: `h2` 28→32px, `blockquote` 20→22px, captions 13→14px, labels 12→13px, small-caps 11→12px, interactive module text 14→16px. The Cultural Revolution timeline year sub-text and death toll text specifically enlarged. Responsive breakpoint 768px: 16→18px. **Design target: standard fullscreen browser on a laptop (1440×900 or similar).**
3. **Nixon/Kennedy debate background expanded** — B5 now includes the date (September 26, 1960), the context (first televised presidential debate in American history), what happened (Nixon had stronger policy arguments but looked pale and sweating; Kennedy looked calm and confident), and the outcome (Kennedy won the election two months later by the narrowest popular-vote margin in a century). The reader should understand what they're looking at before the punchline about radio vs. television.
4. **Dune/Chalamet section reordered** — In Phase 3 of the Hero, the Paul Atreides photo now comes BEFORE the Herbert paragraph, not after. New order: B5 (debate paragraph) → B6a (Chalamet photo + identifying caption: "Paul Atreides, the hero of Frank Herbert's *Dune*.") → B6 (Herbert paragraph about writing Dune as a warning) → B7 (closing paragraph). The photo provides a visual anchor; the caption identifies the character; then Herbert's words explain why it matters.
5. **Hero background image repositioned** — `#heroBg img` gets `object-position: center 30%` so Nixon and Kennedy's faces are visible when the background fades in, not cropped out by the cover scaling.
6. **Routinization pathway diagram enlarged** — `.routine-diagram` max-width increased from `min(80vw, 900px)` to `min(90vw, 1100px)`. The diagram must feel like a full-screen element, not an embedded widget.
7. **Before/after slider initial state changed** — Both sliders now start at 100% original (divider positioned at the far right edge). The user drags LEFT to progressively reveal the retouched version. This is more intuitive: you see the original first, then discover what was erased. Both image pairs also get a shared `aspect-ratio` constraint to prevent misalignment from different source dimensions.
8. **Year counter redesigned as a draggable horizontal slider (revised mid-stream)** — The old vertical scroll-driven counter had two bugs: (a) numbers lost centering as digit count grew, (b) the section often scrolled away before reaching 2020. v5 replaces it with a **horizontal slider that the reader drags by hand** — the slider handle scrubs left→right across an axis from 632 CE to 2024, and is independent of page scroll. As the handle advances, the cumulative death toll above updates and conflict markers along the axis turn red as they are passed. **An earlier v5 draft pinned this module to the viewport and drove progress from scroll; that design was abandoned because (a) markers crammed onto a linear time axis (Karbala/Umayyad cluster left, all modern wars cluster right), and (b) review feedback explicitly asked for a manipulable slider, not a scroll scrub.** Markers now use non-linear positions with alternating above/below labels to prevent overlap, and a fixed-width tabular-num toll number prevents layout shift.
...
| G3 | **Flash-card scroll principle (REWRITTEN v5)** | Global layout + JS architecture | **Implementation: GSAP ScrollTrigger pin.** Every standalone element (prose paragraph, image, blockquote, interactive module) is wrapped in a scroll-trigger zone that pins it to the viewport center. While pinned, the element is fully visible at opacity 1. When the user scrolls past the pin duration, the element fades out (opacity → 0, slight translateY upward) and the next element fades in. **The reader sees exactly ONE element at a time.** Previous content is gone; next content has not appeared. The page behaves like a deck of flash cards — each scroll-step is a new card. **Exceptions:** (a) Scrolly modules (hemicycle vote, CR timeline, inflate number, polarization drift) use their existing sticky-scroll pattern internally, but the MODULE AS A WHOLE must respect the flash-card boundary — when it ends, nothing from the next section is visible. (b) Elements that are logically paired (e.g., an image + its caption) count as ONE card and are pinned together. (c) The Takeaway section (white background) may use slightly relaxed spacing since it is intentionally more readable/editorial, but still no two independent elements visible simultaneously. **CSS fallback:** `margin-bottom` spacing is kept as a no-JS fallback but is NOT the primary mechanism. |
...
| G6 | **Fullscreen-browser design target (NEW v5)** | Global sizing rule | All element sizing, font sizes, and layout calculations assume a **standard fullscreen browser on a laptop** (~1440×900 viewport). Body font-size: **20px** (was 18px). h2: **32px** (was 28px). Blockquote: **22px** (was 20px). Captions: **14px** (was 13px). Section labels/small-caps: **13px** (was 12px). Interactive module body text: **16px** (was 14px). Responsive breakpoint 768px: body **18px** (was 16px). All other font sizes adjust proportionally. |
...
| # | Element | Type | Content |
|---|---------|------|---------|
| B4 | Background image | `opening-JFK-Nixon-debate.jpg` | Fades in as subtle background at 15% opacity, grayscale. **v5: add `object-position: center 30%`** so both candidates' faces are visible, not cropped by the cover scaling. Alt: "John F. Kennedy and Richard Nixon at their first televised presidential debate, Chicago, 1960." |
| B5 | Body paragraph 1 — **EXPANDED (v5)** | Prose | `On September 26, 1960, seventy million Americans tuned in to the first televised presidential debate in the country's history. Richard Nixon had been vice president for eight years. He knew policy cold. On substance, the people who listened on radio thought he won. But Nixon had recently been hospitalized, and it showed — he looked pale, underweight, and was visibly sweating under the studio lights. Kennedy, tanned and composed, looked like he belonged on camera. The television audience was certain Kennedy won. Same words, same arguments. The only difference was the screen. Two months later, Kennedy won the presidency by less than 120,000 votes out of 69 million cast — one of the narrowest margins in American history. The election did not turn on policy. It turned on the fact that one man looked like a leader and the other did not.` |
| B6a | Paul Atreides image — **MOVED BEFORE B6 + ENLARGED (v5)** | `opening-paul-atreides.png` | Film still of Timothée Chalamet as Paul Atreides in Dune (2021). Alt: "Paul Atreides in Dune (2021)." Caption: `Paul Atreides — the hero of Frank Herbert's Dune.` **This image now appears BEFORE the Herbert paragraph, not after it.** The photo provides a visual anchor; the caption identifies the character for readers who haven't seen the film. Then the Herbert text below explains why it matters. **Image is also enlarged ~2× from the standard `.section-image` 680px cap to `min(90vw, 1360px)`, breaking out of the 680px hero-body container via `position:relative; left:50%; transform:translateX(-50%)`** — the photo now reads as a hero element rather than an inline illustration. |
...
| 1.11 | **Routinization pathway diagram — ENLARGED (v5)** | Animated SVG | A single node labeled **Charisma** at top, with three branching paths downward. Path A: `Routinized into Bureaucracy` (labeled `Lenin → Stalin`). Path B: `Fractured into Competing Traditions` (labeled `Muhammad → The Schism`). Path C: `Constrained by Institutions` (labeled `?`, dimmed, with a subtle lock icon). On scroll, paths A and B illuminate sequentially; path C stays dim with a pulsing `?` — it is answered in the Takeaway. **Must fill full viewport per G4. v5: `.routine-diagram` max-width changed from `min(80vw, 900px)` to `min(90vw, 1100px)`.** |
...
| Step 4 | `The Cultural Revolution officially ends. The toll: an estimated 500,000 to 2 million dead, up to 36 million persecuted.` Causal annotation (muted text below death toll — **v5: increase from 14px to 16px**): `Courts suspended. Local governments collapsed. The military was politicized. No institution remained that could stop the violence — because the one man who stood above all institutions had told the people to destroy them.` Then paragraph: `Hitler got the institutions out of his way. Mao went further — he unleashed a force that got out of *his* way.` | Year `1976`, event `Cultural Revolution ends`, intensity 100%, death toll `500,000 – 2,000,000 dead` appears with causal annotation fading in below. **v5: all text in the CR timeline visualization (event text, death toll, causal annotation) increased per G6.** |
...
| 2A.12 | **Before/after slider #1 — Lenin / Trotsky — REVISED (v5)** | Interactive drag-slider. Original photograph (`s2-lenin-original.jpg`, alt: "Lenin speaking at Sverdlov Square, 1920, with Trotsky and Kamenev beside the podium") on the left; retouched photograph (`s2-lenin-removed.jpg`, alt: "The same photograph after Stalin-era retouching, with Trotsky and Kamenev erased") on the right. **v5 change: slider starts at 100% original** — the divider begins at the far RIGHT edge, so the user sees the complete original photo first. Dragging LEFT progressively reveals the retouched version. This is more intuitive: you see what existed, then discover what was erased. Labels below: `Original (1920)` / `← Drag to reveal retouched` (the right-side label is a literal UX hint, since the divider starts at the right edge). Caption: `Lenin at Sverdlov Square, 1920. Trotsky and Kamenev stood beside the podium — until they didn't.` **Both images must share a forced `aspect-ratio` (CSS) to prevent misalignment from different source dimensions. Use the aspect ratio of the shorter image and `object-fit: cover` on the taller one.** |
| 2A.13 | **Before/after slider #2 — Stalin / Yezhov — REVISED (v5)** | Same component and same v5 slider behavior (starts at 100% original, drag left to reveal). Original (`s2-stalin-original.jpg`, alt: "Stalin walking with Nikolai Yezhov along the Moscow-Volga Canal") / retouched (`s2-stalin-removed.jpg`, alt: "The same photograph after Yezhov's execution, with Yezhov erased"). Labels: `Original` / `← Drag to reveal altered` (literal UX hint, same rationale as 2A.12). Caption: `Stalin and secret police chief Nikolai Yezhov at the Moscow Canal. After Yezhov was shot in 1940, he vanished from the photograph too.` **Same forced aspect-ratio rule as 2A.12.** |
...
### 2C — 632 → 2024 draggable horizontal death-toll slider — **REDESIGNED (v5)**

**v5 redesign rationale:** The old vertical scroll-driven counter had two bugs: (a) the large red number lost centering as digit count grew (layout shift), and (b) the section frequently scrolled away before reaching 2020 because the scroll zone was too short relative to the data range. v5 replaces the entire module with a **draggable horizontal slider** that the reader scrubs by hand. The module is a normal-flow block (no sticky pinning, no scroll-driven progress), so the reader plays with it and continues scrolling when ready.

| # | Element | Behavior |
|---|---------|----------|
| 2C.1 | **Slider track (axis)** | A thin horizontal track spanning ~min(90vw, 1100px). Left end labeled `632 CE`, right end labeled `2024`. The progress fill (red) under the handle visualizes the elapsed range. A small `↔ drag` hint sits below. |
| 2C.2 | **Slider handle** | A red-bordered white circular handle the reader drags left/right. Mouse, touch, and keyboard (←/→/Home/End) all advance the position. ARIA `role="slider"` with `aria-valuemin=632` / `aria-valuemax=2024` / `aria-valuenow` updating live. |
| 2C.3 | **Cumulative death toll (large red number)** | Displayed ABOVE the slider, centered. The number updates as the handle is dragged. Toll values are interpolated piecewise between conflict markers, finishing at **1,850,000** at 2024 (matches the Islamic-tree footer caption). **Uses `font-variant-numeric: tabular-nums` and a fixed `min-width` container so digit-count changes don't shift the layout.** |
| 2C.3a | **Current-event readout** | A line directly under the toll showing the active year and event (e.g. `1988 — Iran-Iraq War`). Initial state: `632 CE — Muhammad dies — drag the handle right to advance through history`. |
| 2C.4 | **Conflict markers** | Six markers anchored at non-linear positions on the axis, with labels alternating above/below to prevent overlap: `680 — Karbala (~10,000)` (6%, below), `750 — Umayyad civil wars (~70,000)` (18%, above), `1988 — Iran-Iraq War (~1,000,000)` (62%, below), `2011 — Iraq Civil War (~200,000)` (76%, above), `2014 — Syrian Civil War (~500,000)` (88%, below), `2024 — Yemen Civil War (~150,000+)` (100%, above). Each marker dot turns red and its label brightens once the handle reaches or passes it. |
| 2C.5 | Sub-label | Italic `killed in conflicts with sectarian dimensions — each triggered by its own political crisis, none reducible to religion alone`. Displayed below the slider. |
